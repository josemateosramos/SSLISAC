...
loss_unsuper, loss_val_super, num_it_unsuper, pd_inter_unsuper, pfa_inter_unsuper, \
    ...
    rmse_pos_inter_unsuper = trainNetwork(network, train_it_super, train_it, sigma_sens, theta_mean_min, theta_mean_max, span_min_theta,
                                  span_max_theta, range_mean_min, range_mean_max, span_min_range,
                                  span_max_range, K, S, Delta_f, lamb, Ngrid_angle,
                                  range_grid, pixels_angle, pixels_range, msg_card, refConst, N0,
...
pd_inter = np.concatenate((pd_inter_super, pd_inter_unsuper))
pfa_inter = np.concatenate((pfa_inter_super, pfa_inter_unsuper))
rmse_angle_inter = np.concatenate((rmse_angle_inter_super, rmse_angle_inter_unsuper))
rmse_range_inter = np.concatenate((rmse_range_inter_super, rmse_range_inter_unsuper))
rmse_pos_inter = np.concatenate((rmse_pos_inter_super, rmse_pos_inter_unsuper))
num_iterations = np.concatenate((num_it_super, num_it_unsuper))

plt.figure()
plt.semilogy(num_iterations, 1-np.array(pd_inter), 'o-', label=f'SSL: SL({train_it_super} iterations)+UL')
plt.grid()
...
plt.figure()
plt.semilogx(ser_isac, 1-np.array(pd_isac), 'o-', label=f'SSL: SL({train_it_super} iterations)+UL')
plt.grid()
...
plt.figure()
plt.semilogx(ser_isac, rmse_pos_isac, 'o-', label=f'SSL: SL({train_it_super} iterations)+UL')
plt.grid()
plt.xlabel('Symbol error rate')
